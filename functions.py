...
def visualizedatatemp(dfList, useCase, mostOuterVariable, xtickDict, config): 
    subplottitle = ''
    subplottitlelist = []
...
def visualizedata(dfList, useCase, mostOuterVariable, xtickDict, filterlist, config): 
    fig = plt.figure(figsize=(120, 5.5))
    ...
    if len(classes) > 0:
        title = title + '; hand class segments: ' + classes 
    if len(variants) > 0: 
        title = title + '; hand class variant segments: ' + variants 
    
    if len(factors) > 0:
        title = title + '; hand factor segments: ' + factors
    

    fig.suptitle(title, fontsize=6, fontweight='bold', y=.02)

    i = 0
    
    yticklabelsFlag = True 
    blackFirstFlag = True

    for df in dfList: 
        if len(dfList) > 10 and i > 1:
            yticklabelsFlag = False

        if useCase == 'vsOpen' and 'vsEP' in factors and i > 0:
            yticklabelsFlag = False
        
        if useCase == 'vsOpen' and 'vsMP' in factors and i > 0:
            yticklabelsFlag = False
        ...
        df = df.pivot('variant', mostOuterVariable, 'frequency')
        ax = fig.add_subplot(1, len(dfList), 1 + i)
        ...
        ax = sns.heatmap(df, cmap=subplotcolor, square=True, linewidth=.1, cbar=False, vmin=0, vmax=1, yticklabels=yticklabelsFlag) 
        
        
        #ax.tick_params(axis=u'both', which=u'both', length=0, labeltop=True)
        #ax.set_xlabel(definitions.preFlopUseCases[useCase]['xaxistitle'])
        ax.set_xticklabels(list(xtickDict.values())[i], rotation=45, ha="right", fontsize=8)
        
        if len(filterlist) == 0:
            if blackFirstFlag:
                counter = 0 
                for tick_label in ax.get_yticklabels():
                    for value in definitions.handClasses.values():
                        if tick_label.get_text() in value['variants']:
                            if i > 1: 
                                tick_label.set_color(value['bfmuted'])
                                ax.yaxis.get_ticklines()[counter].set_color(value['bfmuted'])
                                #tick_label.set_fontweight('demi')
                            else:      
                                tick_label.set_color(value['bfcolor'])
                                ax.yaxis.get_ticklines()[counter].set_color(value['bfcolor'])
                            counter += 1
                            continue
                    counter += 1 
                blackFirstFlag = False
                ...
                for tick_label in ax.get_yticklabels():
                    for value in definitions.handClasses.values():
                        if tick_label.get_text() in value['variants']:
                            if i > 1:
                                tick_label.set_color(value['wfmuted'])
                                ax.yaxis.get_ticklines()[counter].set_color(value['wfmuted'])
                                #tick_label.set_fontweight('demi')
                            else: 
                                tick_label.set_color(value['wfcolor'])
                                ax.yaxis.get_ticklines()[counter].set_color(value['wfcolor'])
                            counter += 1
                            continue
                    counter += 1
                blackFirstFlag = True
        ...
        ax.set_title(list(xtickDict.keys())[i], ha="center", fontsize=8, color='#5c4033')
    
        #make labels and tick lines brown along x axis
        for tick_label in ax.get_xticklabels():
            tick_label.set_color('#5c4033')

        for tick_line in ax.xaxis.get_ticklines():
            tick_line.set_color('#5c4033')


        #erase labels and tick lines that are filtered along y axis
        counter = 0 
        
       
        if len(filterlist) > 0:

            for tick_line in ax.get_yticklines():
                tick_line.set_color('#5c4033')

            for tick_label in ax.get_yticklabels():
                if tick_label.get_text() not in filterlist:
                    tick_label.set_visible(False)
                    ax.yaxis.get_major_ticks()[counter].set_visible(False)
